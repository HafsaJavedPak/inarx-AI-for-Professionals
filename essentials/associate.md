...
## Module: "understanding AI" (Lesson 1 — What is AI, really?)
...
1. **Duplicate "Assistance" section removed.** The entire section (heading, three paragraphs, legal/compliance example) is repeated verbatim back-to-back — a copy/paste artifact, not intentional repetition.
2. **Pattern recognition `> CHANGE` — accepted, lightly.** The lesson already has a good spotting example (delivery complaints), but never shows what a human does with the theme afterward. Added one sentence completing that loop rather than a whole new example.
3. **Prediction `> CHANGE` — accepted, lightly.** The invoice example was correct but abstract ("history"). Named the actual inputs (invoice size, payment history, days overdue) so it stops being a placeholder phrase.
4. **Closing `> CHANGE` — accepted, as a genuine improvement.** Right now each of the three jobs is shown with a *different* scenario (tickets, invoices, legal reports), so the reader never sees them connect. Added one short running example — one team, three jobs — since that reinforces "these work together" more than another isolated example would.
...
1. **Redundant "Workplace scenario" under AI removed.** It re-tells the director-in-three-meetings scenario already given two paragraphs above, adding nothing new — pure repetition.
2. **Bridge added at the start of the ML section.** Accepts your coherence `> CHANGE`: right now the lesson jumps straight into a new bank scenario with no link back to "AI is the umbrella term." One sentence fixes it.
3. **"ML vs AI" info box and branch-manager scenario expanded — accepts your CHANGE, done narrowly.** Added one concrete sentence to each rather than turning them into new set-pieces, since the lesson is already dense.
4. **Data Science section's broken "Workplace scenario" fixed.** The heading appears but the scenario text itself is missing — it jumps straight to an orphaned closing sentence ("The insight changes the next promotion plan..."). Folded that sentence into the existing retail intro above, where it completes the thought naturally, and removed the broken heading.
5. **"Connects to ML and AI" box rewritten — accepts your CHANGE.** It asserted a mechanism ("data science feeds ML") without explaining it. Tied it back to the loan-scoring example already established earlier in the lesson, so the "why" is concrete.
6. Closing synthesis (program manager / three stakeholders / tables) left untouched — no issues found.
...
# SECTION (bare `=========` divider, as in your original): Benefits and Trade-offs of Using AI
...
1. **Added a header to each of the four unlabeled sub-topics** (productivity, reducing manual effort, over-reliance, verification-as-fix). Only the first sub-topic ("AI as an Accelerator, Not a Replacement") had a heading — the rest were separated only by `----`, making the lesson hard to scan. This is a navigation fix, not a content rewrite; no sentence of the actual teaching content changes.
2. **Stripped the leftover CMS editor text** ("diting: Stepped timeline," "Delete block") from the activity — authoring-tool residue, not learner content.
3. No `> CHANGE` was flagged on this lesson, and I found no scope, accuracy, or redundancy issues — the accelerator → productivity → effort-reduction → over-reliance → verification arc is tight and doesn't repeat itself.
...
1. **Duplicate "Boundaries" section removed.** The paragraph on boundaries appears twice, back-to-back, word for word — a copy/paste artifact.
2. **Timeline `> CHANGE` — accepted.** The four-step timeline (Goal → Context → Boundaries → Output) had no example attached to any step, feeling abstract right after two subsections full of examples. Added one short example that builds across all four steps, so the reader sees the *same* prompt getting sharper one step at a time.
3. **Meeting-agenda `> CHANGE` — accepted.** In the original, "the goal is to assign responsibilities" reads as a restated goal rather than a boundary, so the four parts weren't cleanly distinguishable. Adjusted the prompt and added a bracketed label after each clause so the 1–4 sequence is visible, not just implied.
4. **Closing "four questions" `> CHANGE` — evaluated critically, partly declined.** You asked for two more detailed examples at the end. By this point the lesson already has the goal accordion, the boundaries list, a worked timeline example, and the meeting-agenda example — a fifth and sixth example risks repetition fatigue in a lesson meant to stay short. Instead of adding new examples, I had the closing four questions explicitly point back to the meeting-agenda example already given, reinforcing the framework through *connection* rather than *addition* — in line with the course's own "meaningful examples over excessive examples" principle.
...
### ORIGINAL / AS-IS (reproduced for completeness — no lesson-content edits applied)
...
Draft the announcement about the office move.

Missing details

Research the topic, write the report, build the slides, and prepare my answers, all at once.

Overly complex request

Tell me the exact resale value my car will have in five years.

Unrealistic expectation

Pull today's live figures straight from our internal dashboard.
...
*(One housekeeping note only, not a content change: the categorization activity still contains leftover "Editing: Categorization / Delete block / Add item" CMS authoring text that should be stripped before publishing — reflected above by simply removing it, since it was never learner-facing content.)*
...
Items (assign each to its correct bucket)
Confirming the exact effective date of a new expense policy

Search first (need a citable source)

Turning call transcripts and survey comments into churn hypotheses

AI first (need synthesis of your own material)

Finding a competitor's current published pricing page

Search first (need a citable source)

Drafting a first-pass options table from your own constraints spreadsheet

AI first (need synthesis of your own material)

Verifying whether an expense category is allowable under internal policy

Search first (need a citable source)

Rewriting a verified policy clause into plain language for frontline staff
...
*(No content change. CMS artifact "Editing: Categorization / Delete block" stripped from the activity as housekeeping only.)*
...
*(No content change — no `> CHANGE` was flagged and none is needed.)*
...
1. **Duplicate "Confidence Is Not Accuracy" section removed** — identical paragraph repeated back-to-back.
2. **Bridge added at the top**, connecting this lesson to "Why AI Can Be Wrong" — the two are closely related failure modes and, as written, read as unconnected.
3. **`> CHANGE` on the Section 14(b) example — evaluated critically, partly accepted.** The sentence itself isn't vague — it's a well-built example doing double duty as both a fake citation *and* an invented statistic in one compact line, tied directly to the accordion items above it. Making the fabricated claim longer would blunt the lesson's real point, that one confidently-worded sentence is often all it takes to slip past review. What was actually missing was workplace framing: the example drops in with no sense of where it would show up. Added that framing (a director's quarterly update) before and after, which addresses "too short" without diluting the example itself.
...
1. **Bridge added at the top**, distinguishing bias from the previous two lessons — otherwise a reader could conflate "bias" with "hallucination," since both involve a confident, wrong-feeling output.
2. **First `> CHANGE` — accepted.** "Bias Starts in the Data" only had one illustrative example (hiring/promotion history). Added a short, professional list naming two or three other common types of biased data, grounding the abstract idea without turning into a taxonomy.
3. **Second `> CHANGE` — evaluated critically, mostly declined.** The accordion directly beneath this paragraph already supplies three concrete professional examples (tone, omitted perspectives, hiring bias). Adding more examples in the prose would duplicate what the accordion is about to show. Added one short teaser sentence that sets up the accordion instead — meeting the spirit of your request without repeating content the lesson already has.
...
1. **Bridge added at the top**, connecting from the Bias lesson — both are about risks tied to what goes *into or through* an AI tool, but from different angles (fairness of outputs vs. confidentiality of inputs).
2. **Content gap flagged, not invented.** "Bringing It Together" promises "the questions below check that both are clear," but no questions or activity actually follow in the source material — this reads like content lost in export rather than an intentional ending. I've left a bracketed editorial note rather than fabricating an activity to fill it, since guessing at what should go there risks putting words in the course's mouth.
3. No other issues found — the personal caution → organizational obligation → habit-forming routine → recap progression is sound and appropriately non-technical.
...
[EDITORIAL NOTE — not part of the lesson: the source text ends this section with "The questions below check that both are clear," but no check or activity follows anywhere in the material. This looks like a content gap from export rather than an intentional ending. Recommend confirming with the source file and inserting the intended check here, rather than having this review invent one.]
